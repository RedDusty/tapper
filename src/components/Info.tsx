...
function latencyShow(
  latency: number | "?" | "Offline" | "Reconnect",
...
function Info() {
  const [latency, setLatency] = useState<
    number | "?" | "Offline" | "Reconnect"
  >("?");
  const [online, setOnline] = useState<number | "?">("?");
  const { t } = useTranslation();

  const { code, inLobbyPlayers, maxPlayers } = useTypedSelector(
    (state) => state.lobby
  );
  ...
  useEffect(() => {
    ...
    getSocket().on("ONLINE_UPDATE", (usersCount) => {
    ...
    getSocket().io.on("reconnect", (data) => {
      setLatency("Reconnect");
    });
    return () => {
      clearInterval(interval);
      getSocket().off("ONLINE_UPDATE");
      getSocket().off("SERVER_PING");
    };
  }, []);
  return (
