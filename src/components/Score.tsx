  ...
  return (
    <div className="panelWidth my-0 mx-auto">
      {users.map((user, index) => {
        return <RenderUser user={user} key={user.firstLogin + index} index={index + 1} />;
      })}
    </div>
  );
}
...
const RenderUser: ({ user, index }: { user: userInfoType; index: number }) => JSX.Element = ({ user, index }) => {
  const placeUnder = () => {
    if (index === 1) {
  ...
  return (
    <div className={`shadow-md rounded-xl m-2 min-w-min max-w-max mx-auto ${placeUpper()}`}>
      <div className={`w-full h-full rounded-xl p-2 flex items-center ${placeUnder()}`}>
